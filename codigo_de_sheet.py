...
if __name__ == "__main__":
    image_path = "img/photo_1.png"
    detected_peaks = detect_highest_peaks(image_path)

    # Extract first, last, and lowest coordinates
    first_coord = (detected_peaks[1][0], detected_peaks[0][0])
    last_coord = (detected_peaks[1][-1], detected_peaks[0][-1])
    lowest_coord = detected_peaks[1][np.argmin(detected_peaks[0])], np.min(detected_peaks[0])

    print("First coordinate:", first_coord)
    print("Last coordinate:", last_coord)
    print("Lowest coordinate:", lowest_coord)
